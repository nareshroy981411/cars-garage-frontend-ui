...
const CartPage = () => {
  ...
  const handleQuantityChange = (itemId, newQuantity) => {
    ...
    if (newQuantity > item.stockQuantity) {
      toast.error(`Only ${item.stockQuantity} items in stock for "${item.name}"`)
      return
    }
                    ...
                    <div className="divide-y divide-gray-200">
                      {cartItems.map(item => (
                        <div key={item.id} className="p-4 md:flex items-center">
                          ...
                          <h3 className={`text-sm ${item.stockQuantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {item.stockQuantity > 0 ? `${item.stockQuantity} In stock` : 'Out of stock'}
                    </h3>
                            <h3 className="text-lg font-semibold text-gray-800">{item.name}</h3>
                            <p className="text-gray-600 text-sm line-clamp-2">{item.description}</p>
                            <p className="text-blue-600 font-medium mt-2">₹{Number(item.price).toFixed(2)}</p>
                          </div>
